...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string) => {
    const data = await api.post<AuthResponse>("/authentication/login", { email, password }, { headers: { "X-Request-Id": uuidv4() } });
    localStorage.setItem("access_token", data.accessToken);
    api.setAuthToken(data.accessToken);
    if (data?.refreshToken?.token) {
      localStorage.setItem("refresh_token", data.refreshToken.token);
  ...
  const register = useCallback(async (payload: { firstName: string; lastName: string; email: string; password: string }) => {
    const data = await api.post<AuthResponse>("/authentication/register", payload, { headers: { "X-Request-Id": uuidv4() } });
    localStorage.setItem("access_token", data.accessToken);
    api.setAuthToken(data.accessToken);
    if (data?.refreshToken?.token) {
      localStorage.setItem("refresh_token", data.refreshToken.token);
