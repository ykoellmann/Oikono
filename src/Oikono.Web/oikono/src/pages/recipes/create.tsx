...
export default function RecipeCreatePage() {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      // Minimal step editor: store content as plain text/markdown
      ...
      // POST to backend (adjust path if needed)
      // If your backend expects JSON, replace with a JSON payload and upload endpoint for files.
  ...
  return (
    <PageLayout title="Neues Rezept">
      <form className="flex flex-col gap-6 max-w-3xl" onSubmit={form.handleSubmit(onSubmit)}>
        ...
        <section className="grid gap-3">
          ...
          {previews.length > 0 && (
            <div className="grid grid-cols-3 gap-2">
              {previews.map((src, i) => (
                <img key={i} src={src} alt={`preview-${i}`} className="aspect-video w-full object-cover rounded border" />
              ))}
            </div>
          )}
        </section>
          ...
          <div className="flex flex-col gap-2">
            {ingArray.fields.map((field, idx) => (
              <div key={field.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-2 items-center">
                ...
                <Input placeholder="Menge" type="number" step="0.01" className="w-28" {...form.register(`ingredients.${idx}.amount` as const)} />
                <Input placeholder="Einheit" className="w-28" {...form.register(`ingredients.${idx}.unit` as const)} />
                <Button type="button" variant="ghost" size="icon" onClick={() => ingArray.remove(idx)}>
                  <Trash2 className="h-4 w-4" />
                </Button>
              </div>
            ))}
            <Button type="button" variant="outline" onClick={() => ingArray.append({ name: "", amount: 1, unit: "" })}>
            ...
            {stepsArray.fields.map((field, idx) => (
              <div key={field.id} className="border rounded p-3 space-y-2">
                <div className="flex gap-1 justify-end">
                  <Button type="button" variant="ghost" size="icon" onClick={() => moveStep(idx, -1)} disabled={idx === 0}><ArrowUp className="h-4 w-4"/></Button>
                  <Button type="button" variant="ghost" size="icon" onClick={() => moveStep(idx, 1)} disabled={idx === stepsArray.fields.length - 1}><ArrowDown className="h-4 w-4"/></Button>
                  <Button type="button" variant="ghost" size="icon" onClick={() => stepsArray.remove(idx)}><Trash2 className="h-4 w-4"/></Button>
                </div>
                {/* Simplified editor: plain textarea for now; can be replaced with Lexical later */}
                <Textarea rows={4} placeholder={`Schritt ${idx + 1} beschreiben…`} {...form.register(`steps.${idx}.content` as const)} />
              </div>
            ))}
